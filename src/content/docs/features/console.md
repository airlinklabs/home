---
title: "Console & Logs"
description: "Live terminal output, server logs, and resource monitoring."
section: "Features"
order: 17
---
...
The console streams real-time output from game server containers over WebSocket.
...
The console page includes power controls (start, stop, restart, kill) with the same permission checks as the API.

## Server Logs

The logs page (`views/user/server/logs.ejs`) gives access to server log files. Logs are fetched from the daemon's file API and displayed in the browser.

### Log Access

Logs are read from common log paths within the server directory. The exact paths depend on the server's image definition and configuration.
