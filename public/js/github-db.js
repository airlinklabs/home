...
(function () {
      ...
      if (window.initSqlJs) {
        window.initSqlJs({
          locateFile: function (f) {
            return "https://cdnjs.cloudflare.com/ajax/libs/sql.js/1.14.2/" + f;
          },
        }).then(resolve).catch(reject);
        return;
      }
      var s = document.createElement("script");
      s.src = "https://cdnjs.cloudflare.com/ajax/libs/sql.js/1.14.2/sql-wasm.js";
      s.onload = function () {
        window.initSqlJs({
          locateFile: function (f) {
            return "https://cdnjs.cloudflare.com/ajax/libs/sql.js/1.14.2/" + f;
  ...
  async function fetchGitHubData() {
    ...
    var results = await Promise.allSettled([
      ghFetch("https://api.github.com/repos/" + PANEL_REPO + "/commits?per_page=20"),
      ghFetch("https://api.github.com/repos/" + DAEMON_REPO + "/commits?per_page=20"),
      ghFetch("https://api.github.com/repos/" + PANEL_REPO + "/contributors?per_page=100"),
      ghFetch("https://api.github.com/repos/" + DAEMON_REPO + "/contributors?per_page=100"),
    ]);

    var panelCommits  = results[0].status === "fulfilled" ? results[0].value : [];
    var daemonCommits = results[1].status === "fulfilled" ? results[1].value : [];
    var panelContribs  = results[2].status === "fulfilled" ? results[2].value : [];
    var daemonContribs = results[3].status === "fulfilled" ? results[3].value : [];
    ...
    var allCommits = []
      .concat(panelCommits.map(function (c) { return normalizeCommit(c, "panel"); }))
      .concat(daemonCommits.map(function (c) { return normalizeCommit(c, "daemon"); }));
  ...
  function renderCommits(commits) {
    ...
    commits.slice(0, 14).forEach(function (c) {
      ...
      var repoClass = c.repo === "panel" ? "commit-tag--panel" : "commit-tag--daemon";
      var sha = (c.sha || "").slice(0, 7);

      html += '<a href="' + (c.html_url || "#") + '" class="hub-list-row" target="_blank" rel="noopener">';
      html += '<div class="commit-row-main">';
      html += '<div class="commit-row-meta">';
      html += '<span class="commit-tag ' + repoClass + '">' + repoLabel + '</span>';
      if (sha) html += '<span class="commit-sha">' + sha + '</span>';
      html += '</div>';
      html += '<p class="hub-list-title">' + escHtml(msg) + '</p>';
      html += '<p class="hub-list-description">' + escHtml(c.author_name || "unknown") + '</p>';
      html += '</div>';
      html += '<span class="hub-list-meta">' + date + '</span>';
      html += '<span class="hub-arrow" aria-hidden="true"><svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg></span>';
      html += '</a>';
    });
    el.innerHTML = html;
  }
  ...
  function renderContributors(contributors) {
    ...
    contributors.forEach(function (c) {
      html += '<a href="' + (c.html_url || "#") + '" class="contrib-card" target="_blank" rel="noopener">';
      if (c.avatar_url) {
        html += '<img src="' + c.avatar_url + '&s=80" alt="" width="40" height="40" loading="lazy">';
      } else {
        html += '<span class="contrib-avatar-placeholder"></span>';
      }
      html += '<div>';
      html += '<p class="contrib-name">' + escHtml(c.login || c.name || "unknown") + '</p>';
      var n = c.contributions;
      html += '<p class="contrib-count">' + n + ' contribution' + (n !== 1 ? 's' : '') + '</p>';
      html += '</div>';
      html += '</a>';
    });
    el.innerHTML = html;
  ...
  function showCommitSkeletons() {
    ...
    for (var i = 0; i < 6; i++) {
      ...
      html += '<div class="commit-row-main"><div class="skeleton-line skeleton-line--wide"></div><div class="skeleton-line skeleton-line--narrow"></div></div>';
  ...
  function showContribSkeletons() {
    ...
    for (var i = 0; i < 8; i++) {
      ...
      html += '<span class="contrib-avatar-placeholder"></span>';
      html += '<div><div class="skeleton-line skeleton-line--name"></div><div class="skeleton-line skeleton-line--count"></div></div>';
      html += '</div>';
    }
    el.innerHTML = html;
  }
  ...
  async function main() {
    ...
    try {
      ...
      if (raw) {
        ...
        if (age < LS_TTL && cached.commits && cached.commits.length) {
          ...
          if (age > 2 * 60 * 1000) {
            fetchGitHubData().then(function (data) {
    ...
    try {
      ...
      var commitRows = safeQuery(db, "SELECT sha, repo, message, author_name, author_date, author_avatar, html_url FROM commits ORDER BY author_date DESC");
      var contribRows = safeQuery(db, "SELECT login, name, avatar_url, html_url, contributions, bio, company FROM contributors ORDER BY contributions DESC");

      if (commitRows.length > 0 || contribRows.length > 0) {
        dbLoaded = true;
        var commits = commitRows.map(function (r) {
          return { sha: r[0], repo: r[1], message: r[2], author_name: r[3], author_date: r[4], author_avatar: r[5], html_url: r[6] };
        });
        var contribs = contribRows.map(function (r) {
          return { login: r[0], name: r[1], avatar_url: r[2], html_url: r[3], contributions: r[4] };
        });
    ...
    fetchGitHubData()
      ...
      .catch(function (err) {
        ...
        if (!dbLoaded && cached) {
          ...
          if (commitEl) commitEl.innerHTML = '<p class="hub-empty">Could not load commits. Check back soon.</p>';
          if (contribEl) contribEl.innerHTML = '<p class="hub-empty">Could not load contributors. Check back soon.</p>';
        }
      });
  }
