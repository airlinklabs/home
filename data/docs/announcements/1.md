...
<(flow title="Daemon authentication" steps="Panel:Generate timestamp+nonce,Panel:Sign request with HMAC-SHA256,Panel:Send to daemon,Daemon:Check IP allowlist,Daemon:Verify Basic Auth,Daemon:Validate HMAC signature")>
...
<(counter value=6 label="auth checks per request")>
